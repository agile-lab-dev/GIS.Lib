...
class GeocodeManagerSpec extends FlatSpec with Matchers with EitherValues with BeforeAndAfterAll {
  ...
  val geocodeConf: Config = conf.getConfig(GEOCODE.value)
  val manager: GeocodeManager = GeocodeManager(geocodeConf)
  ...
  "Reverse geocoding on Andorra" should "work" in {
    ...
    val randomPlaceInAndorraActual: ReverseGeocodingResponse = manager.reverseGeocode(point).right.value
  ...
  "Reverse geocoding on Italy" should "has correct postalcode valued" in {
    ...
    val albignasego: ReverseGeocodingResponse = manager.reverseGeocode(point).right.value
  ...
  "Reverse geocoding in east Milan" should "work" in {
    ...
    val milan: ReverseGeocodingResponse = manager.reverseGeocode(point).right.value
  ...
  "Reverse geocoding on Italy" should "has correct house number valued" in {
    ...
    val corsoSaccoEVanzettiActual: ReverseGeocodingResponse = manager.reverseGeocode(point).right.value
  ...
  "Reverse geocoding on Italy" should "not query street index" in {
    ...
    val corsoSaccoEVanzettiActual: ReverseGeocodingResponse =
      manager.reverseGeocode(point, ReverseGeocoder.indices.filter(v => !v.equals(Streets))).right.value
  ...
  "Reverse geocoding on Italy" should "not query the house numbers index" in {
    ...
    val corsoSaccoEVanzettiActual: ReverseGeocodingResponse =
      manager.reverseGeocode(point, ReverseGeocoder.indices.filter(v => !v.equals(HouseNumbers))).right.value
  ...
  "GeocodeManager" should "retrieve OSM index from file and reverse geocode a Turin coordinate" in {
    val managerIndexRetriever =
      GeocodeManager(ConfigFactory.load("reference-osm-file-index.conf").getConfig(GEOCODE.value))
  ...
  private def reverseGeocodeTurin(geocodeManager: GeocodeManager): Unit = {
    ...
    val viaAzziActual: ReverseGeocodingResponse = geocodeManager.reverseGeocode(point).right.value
  ...
  //    val lat = 43.861599
  //    val lon = 18.365856
  //    val randomPlaceInSarajevoActual: Address = manager.reverseGeocode(lat, lon)
  //
  ...
  "Reverse geocoding on Germany" should "work" in {
    ...
    val leipzig = manager.reverseGeocode(point).right.value
