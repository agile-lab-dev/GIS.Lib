...
class GraphHopperSpec extends FlatSpec with Matchers with BeforeAndAfterAll {
  ...
  override def beforeAll(): Unit = {
    ...
    val pbfFilePath: String =
      s"$basePath/graphHopperSource/italy-latest.osm.pbf"
  ...
  "trip" should "retrieve points sorted by timestamp" in {
    ...
    val gpsPoint: List[GPXEntry] = List(
      new GPXEntry(37.9637, 12.6404, 1563855394000L),
      new GPXEntry(37.9662, 12.6403, 1563855520000L),
      new GPXEntry(37.9666, 12.6439, 1563855590000L),
      new GPXEntry(37.9649, 12.6448, 1563855660000L),
      new GPXEntry(37.9631, 12.6459, 1563855740000L),
      new GPXEntry(37.962, 12.6481, 1563855800000L),
      new GPXEntry(37.9615, 12.6486, 1563855810000L)
    )
    ...
    val expectedTimestamp =
      gpsPoint.zip(gpsPoint.tail).map { case (a, b) => b.getTime - a.getTime }
  ...
  "test with point to calculate distance " should "calculate distance between points" in {
    ...
    val gpsPoint: List[GPXEntry] = List(
      new GPXEntry(42.4599, 12.3813, 1568268784000L),
      new GPXEntry(42.46, 12.3823, 1568268786000L)
    )
