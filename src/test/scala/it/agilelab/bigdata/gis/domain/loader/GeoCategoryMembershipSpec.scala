...
class GeoCategoryMembershipSpec extends FlatSpec with Matchers {
  ...
  "Config for the GeometryMembershipManager" should "be successfully loaded" in {
    ...
    result.get.categoryInfo should contain(
      CountryCfg(Map(2 -> GeoMetadataCfg("Code"), 3 -> GeoMetadataCfg("Name")), 3))
  }

  "GeometryMembershipManager" should "not be initialize if categories do not match" in {
    val aWrongPathCfg = CategoriesCfg(Paths.get("src/test/resources/osm/aPathNotExisting"), Seq(CustomCfg("aLabel", Map.empty, 0)))
  ...
  "GeometryMembershipManager" should "load Country Category successfully" in {
    val categoriesCfg = CategoriesCfg(Paths.get("src/test/resources/osm/categories"), Seq(CountryCfg(Map.empty, 0)))
    val sut = GeometryMembershipInfoManager
  ...
  "GeometryMembershipManager.getGeoMembershipInfoOf[Point]" should
    ...
    val res = for {
      cfg <- CategoriesCfg.load
      sut <- GeometryMembershipInfoManager(cfg)
      point <- Try(new GeometryFactory().createPoint(new Coordinate(18.3884, 44.1812)))
    } yield (sut.getGeoMembershipInfoOf(InputCategory.Country, point))
  ...
  "GeometryMembershipManager.getGeoMembershipInfoOf[Point]" should
    ...
    val res = for {
      cfg <- CategoriesCfg.load
      sut <- GeometryMembershipInfoManager(cfg)
      point <- Try(new GeometryFactory().createPoint(new Coordinate(7.6756, 45.0735)))
    } yield (sut.getGeoMembershipInfoOf(InputCategory.Country, point))
  ...
  "GeometryMembershipManager.getFirstGeoMembershipInfoOf[Point]" should
    ...
    val res = for {
      cfg <- CategoriesCfg.load
      sut <- GeometryMembershipInfoManager(cfg)
      point <- Try(new GeometryFactory().createPoint(new Coordinate(1.66921, 42.50555)))
  ...
  "GeometryMembershipManager.getGeoMembershipInfoOf[MultiPoint]" should
    ...
    val res = for {
      cfg <- CategoriesCfg.load
      sut <- GeometryMembershipInfoManager(cfg)
      line <- Try(new GeometryFactory().createMultiPoint(
        Seq(coord_spain, coord_toulouse).toArray))
  ...
  "GeometryMembershipManager.getGeoMembershipInfoOf[MultiPoint]" should
    ...
    val res = for {
      cfg <- CategoriesCfg.load
      sut <- GeometryMembershipInfoManager(cfg)
      points <- Try(new GeometryFactory().createMultiPoint(
    ...
    res.get.map(_.geometryLabel) should (contain("Andorra_AL2") and contain("Bosnia and Herzegovina"))
  }
  ...
  "GeometryMembershipManager.getFirstGeoMembershipInfoOf[MultiPoint]" should
    ...
    res shouldBe a[Success[_]]
    res.get.size shouldBe 2
    res.get.map(_.geometryLabel) should (contain("Andorra_AL2") or contain("Bosnia and Herzegovina"))
  }
  ...
  "GeometryMembershipManager.getFirstGeoMembershipInfoOf[MultiPoint] - STRICT" should
    ...
    val res = for {
      ...
      points <- Try(new GeometryFactory().createMultiPoint(
        Seq(coord_andorra, coord_bosnia, coord_toulouse).toArray))
    } yield sut.getGeoMembershipInfoOf(InputCategory.Country, points, strictMembership = true)
  ...
  "GeometryMembershipManager.getFirstGeoMembershipInfoOf[MultiPoint] - STRICT" should
    ...
    val res = for {
      ...
      sut <- GeometryMembershipInfoManager(cfg)
      points <- Try(new GeometryFactory().createMultiPoint(
        Seq(coord_andorra, coord_bosnia).toArray))
    } yield sut.getGeoMembershipInfoOf(InputCategory.Country, points, strictMembership = true)
  ...
  "GeometryMembershipManager.getGeoMembershipInfoOf[MultiPoint]" should
    ...
    val res = for {
      ...
      runnable = new Runnable {
        override def run(): Unit = sut.getGeoMembershipInfoOf(InputCategory.Country, point, strictMembership = true)
      }
    } yield runnable
