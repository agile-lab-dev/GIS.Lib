...
object OSMStreetEnriched{

  def decorateWithNumbers(roadEL: OSMStreetEnriched, addressEls: Seq[OSMAddress]): OSMStreetEnriched = {
    val numbersEls =
      addressEls.map(
        osmAddress =>
...
case class OSMStreetEnriched(pointsArray: Array[Double],
  ...
  override def compareToSameClass(o: scala.Any): Int = {
    val s: OSMStreetEnriched = o.asInstanceOf[OSMStreetEnriched]
  ...
  def getXIndex(n: Int) = n*dimensionPoints
  def getYIndex(n: Int) = n*dimensionPoints+1
  ...
  override def isEmpty: Boolean = this.pointsArray.size == 0
  ...
  override def equalsExact(other: Geometry, tolerance: Double): Boolean = {
    if (!this.isEquivalentClass(other) || !other.isInstanceOf[OSMStreetEnriched]) false
    else {
      val otherLine = other.asInstanceOf[OSMStreetEnriched]
  ...
  override def getNumPoints: Int = pointsArray.size / dimensionPoints
