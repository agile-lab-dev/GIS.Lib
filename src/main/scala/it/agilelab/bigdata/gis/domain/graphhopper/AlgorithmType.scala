...
object AlgorithmType {
  ...
  case object DIJKSTRA_ONE_TO_MANY extends AlgorithmType { lazy val value: String = Parameters.Algorithms.DIJKSTRA_ONE_TO_MANY }
  ...
  def fromValue(v: String): AlgorithmType =
    v match {
      ...
      case _ => throw new IllegalArgumentException(s"$v is not a valid algorithm type, the algorithm supported are ${values.mkString(",")}")
    }
}