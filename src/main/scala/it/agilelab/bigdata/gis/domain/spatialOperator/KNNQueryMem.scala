...
import it.agilelab.bigdata.gis.domain.knnJudgement.{GeometryDistanceOrdering, GeometryKnnJudgementS, KnnJudgementUsingIndexS}
...
object KNNQueryMem extends Serializable {
		...
		if(useIndex) {
			...
			if(spatialList.index == null) {
				throw new NullPointerException(
					"Need to invoke buildIndex() first, indexedCollectionNoId is null")
			}
		...
		else
		{
			if(spatialList.rawSpatialCollection == null) {
				throw new IllegalArgumentException(
					"You can't invoke raw search if you built index, remove buildIndex from your load procedure")
			}
			else {
				val tmp =
					GeometryKnnJudgementS.invoke(spatialList.rawSpatialCollection.toIterator, queryCenter, k)
	...
	def SpatialKnnQueryJava[T <: Geometry](spatialList: GeometryList[T],
																				 queryCenter: Point,
																				 k: Int,
