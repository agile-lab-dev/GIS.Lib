...
class OSMPlaceShapeLoader() extends Loader[OSMPlace] {
  override def loadFile(source: String): Iterator[(Array[AnyRef], Geometry)] = {
    ...
    val poligons: Seq[(Geometry, SimpleFeature)] = if (source.endsWith("places_a_free_1.shp")) {
    ...
    poligons.map { case (geometry, list) => (Array[AnyRef](list), geometry) }.toIterator
  }

  protected def objectMapping(fields: Array[AnyRef], line: Geometry): OSMPlace = {

    // TODO this doesn't work, we should use fields name
