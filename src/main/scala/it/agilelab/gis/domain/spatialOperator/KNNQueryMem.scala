...
object KNNQueryMem extends Serializable {
    ...
    if (useIndex) {
      ...
      if (spatialList.rawSpatialCollection == null) {
        throw new IllegalArgumentException(
          "You can't invoke raw search if you built index, remove buildIndex from " +
            "your load procedure")
      } else {
  ...
  /** Spatial knn query. Results having a distance greater than d are omitted.
    ...
    * @return the list
    */
  def spatialKnnQueryWithMaxDistance[T <: Geometry](
      spatialList: GeometryList[T],
      queryCenter: Point,
      k: Int,
      distance: Double,
  ...
  /** Spatial query returning the indexed geometries intersecting the circle having as center the queryCenter and
    ...
    * @return the list
    */
  def spatialQueryWithMaxDistance[T <: Geometry](
      spatialList: GeometryList[T],
      queryCenter: Point,
      distance: Double,
      useIndex: Boolean = true
  ...
  def spatialKnnQueryJava[T <: Geometry: ClassTag](
      spatialList: GeometryList[T],
      queryCenter: Point,
      k: Int,
      useIndex: Boolean = true
